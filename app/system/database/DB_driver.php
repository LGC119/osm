...
class CI_DB_driver {
	...
	{
		include(BASEPATH.'../'.APPPATH.'config/database.php');
	    ...
	    foreach($_master_slave_relation as $key => $val) {
	        if($key == $this->group_name) {
	            $db_proxy = true;
	            $db_master_group = $this->group_name;
	            break;
	        }
	        foreach($val as $v) {
	            if($v == $this->group_name) {
	                $db_proxy = true;
	                $db_master_group = $key;
	                break;
	            }
	    ...
	    if($this->is_write_type($sql)||$this->db_force_master == TRUE) {
        	$proxy_setting = isset($db[$db_master_group]) ? array($db_master_group => $db[$db_master_group]) : false;
	    } else {
	        $CI = & get_instance();
	        foreach($_master_slave_relation[$db_master_group] as $val) {
	            if(isset($CI->{'conn_'.$val}) && is_resource($CI->{'conn_'.$val})) {
	                $proxy_setting = array($val => $db[$val]);
	            }
	        }
	        ...
	        $proxy_setting = isset($db[$db_slave_group_name]) ? array($db_slave_group_name => $db[$db_slave_group_name]) : false;
	    }
