...
//        return $this ->db ->last_query();
    ...
    public function create_rule($data){
        $ruleTab = 'wx_rule';
        $keywordTab = 'wx_keyword';
        $mediaTab = 'rl_wx_media_rule';
        $ruleTag  = 'rl_wx_rule_tag';
        // 添加到规则表中
    ...
    public function select_other($aid){
        $account = 'wx_account';
        $sql = "SELECT subscribed_reply,nokeyword_reply,type_reply,type2_reply FROM ".$this ->db ->dbprefix($account)."
                WHERE id='$aid'";
        $data = $this ->db ->query($sql)->result_array();
        if($data)
            return $data[0];
    ...
    **/
    ...
    {
        $joinTable = 'rl_wx_user_tag';
        $userTable = 'wx_user';
		$joinStr = 'rwut.wx_user_id=wu.id';
		$whereArr = array(
			'rwut.wx_aid' =>$wx_aid,
			'wu.id'       =>$wx_user_id
		);
    ...
    // 更新其他回复规则
    public function update_other_rule($aid,$subscribedReply,$noKeywordReply,$type='',$type2=''){
        $account = 'wx_account';
        $sql = "UPDATE ".$this ->db ->dbprefix($account)." SET
                subscribed_reply='$subscribedReply',nokeyword_reply='$noKeywordReply',type_reply='$type',type2_reply='$type2'
                WHERE id='$aid'";
//        echo $this ->db ->last_query();
    ...
    public function select_other_rule($wx_aid){
        $sql = "SELECT nokeyword_reply FROM ".$this ->db ->dbprefix('wx_account')."
                WHERE id='$wx_aid'";
        $data = $this ->db ->query($sql)->result_array();
        if($data)
