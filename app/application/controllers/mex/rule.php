...
class Rule extends ME_Controller
{
    ...
    public function __construct(){
        ...
        $this ->load ->model('mex/rule_model','rule');
...
//        $newData['page'] = array();
    ...
    public function insert_rule(){
        ...
        $data1['aid'] = $data['aid'] = $this ->wx_aid;
...
//        var_dump($data);
    ...
    public function select_other(){
		...
		if($data['type_reply'] == 'image'){
			$mediaid = $data['subscribed_reply'];
			$this->load->model('mex/media_model','media');
			$data2 = $this->media->get_media_all($mediaid);
			$data['data'] = $data2;
		}
		if($data['type2_reply'] == 'image'){
			$mediaid2 = $data['nokeyword_reply'];
			$data3 = $this->media->get_media_all($mediaid2);
			$data['data2'] = $data3;
		}
		if($data['type_reply'] == 'news'){
			$mediaid = $data['subscribed_reply'];
			$this->load->model('mex/media_model','media');
			$data2 = $this->media->get_media_all($mediaid);
			$data['data'] = $data2;
		}
		if($data['type2_reply'] == 'news'){
			$mediaid2 = $data['nokeyword_reply'];
			$this->load->model('mex/media_model','media');
			$data3 = $this->media->get_media_all($mediaid2);
			$data['data2'] = $data3;
		}
		if($data['type_reply'] == 'articles'){
			$mediaid = $data['subscribed_reply'];
			$this->load->model('mex/media_model','media');
			$data2 = $this->media->get_media_all($mediaid);
			$this->load->model('mex/send_model','send');
			$data['data'] = $this->send->get_articles_info($data2[0]['articles']);
		}
		if($data['type2_reply'] == 'articles'){
			$mediaid2 = $data['nokeyword_reply'];
			$this->load->model('mex/media_model','media');
			$data3 = $this->media->get_media_all($mediaid2);
			$this->load->model('mex/send_model','send');
			$data['data2'] = $this->send->get_articles_info($data3[0]['articles']);
		}
        if($data)
    ...
    public function update_other_rule(){
		...
		$sendValue2 = $this->input->post("sendValue2");
        if($sendValue2['sType'] == 'text'){
            $value2 = $noKeywordReply;
        } else {
    		$value2 = $sendValue2['sValue'];
        }
		// $sendValue2['sType']
		// $value2 = $sendValue2['sValue'][0]
        $status = $this ->rule ->update_other_rule($aid,$value,$value2,$type,$sendValue2['sType']);
        if($status)
