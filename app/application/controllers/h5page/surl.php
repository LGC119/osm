...
class Surl extends CI_Model
{
    ...
    public function access() 
    {
        ...
        $url = $this->db->get_where($this->tb['surl'], $where)->result_array();
        //redirect($url[0]['original_url'] . $query);
        if ($url) {
            //echo $url[0]['original_url'] . $query;
            $pre = '';
            if (!empty($input) ) {
                if ( preg_match('*\?*', $url[0]['original_url'])) {
                    $pre = '&';
                } else {
                    $pre = '?';
                }
            }
            redirect($url[0]['original_url'] . $pre . $query);
        } else {
            echo 'no url';
        }
    }
     ...
     * @return string 缩略字符
     */
    private function _urlShort($url)
    {
        $url= crc32($url);
