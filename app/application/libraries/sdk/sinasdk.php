...
class SaeTOAuthV2
{
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		if ( !empty($this->remote_ip) ) {
			...
			if ( !defined('SAE_ACCESSKEY') ) {
				$svrip = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1';
				$headers[] = "API-RemoteIP: " . $svrip;
			}
		}
...
//		);
		$str = '{
		   ...
		   "text":{
			  "content":"sdnkldfklgnkfdngknekl"
		   },
			"msgtype":"text"
		}';
//		$json = array();
//		$json["touser"] = array("3811256246");
//		$json["text"] = array(
//			"content"=>"dfsfsf"
//		);
//		$json["msgtype"] = "text";
		...
		$data = $this->oauth->post( 'https://m.api.weibo.com/2/messages/sendall.json?access_token='.$this->oauth->access_token, $json);
	}
	...
	/**
	 * @author 董开顺
	 * 批量获取用户的粉丝数、关注数、微博数 | 新增
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/users/counts}
	 *
	 * @param string $uids 需要获取数据的用户UID，多个之间用逗号分隔，最多不超过100个。
	 * @return array
	 */
	function user_counts( $uids )
	{
		$params = array();
	 ...
	 * 设置私信消息推送格式
	**/
	function push_set_format ($format = 'JSON') 
	{
		$params = array(
			'format' => $format
		);

		return $this->oauth->post( 'eps/push/set_format', $params);
	}
	...
	function reply_message ($type, $data, $receiver_id, $save_sender_box = 1) 
	{
		$params = array (
			'type' => $type, 
			'data' => $data, 
		...
		return $this->oauth->post('https://m.api.weibo.com/2/messages/reply.json', $params);
	}
}
