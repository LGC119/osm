...
define(['me'], function (me) {
	me.controller('WeiboUserGroupController', ['$scope', '$sce', '$http' , 'WeiboUserGroup' , function ($scope, $sce, $http, WeiboUserGroup) {
		$scope.groupData = {};
		...
		$scope.getGroups = function () {
			...
			WeiboUserGroup.getList(params, function (res) {
				if (res.code == 200) {
		...
		$scope.create_group = function(){
			...
			$http.post(
				_c.appPath+'meo/wb_group/insert_group',
				{
					name     : name,
					desc     : desc,
					feature  : feature,
...
//                    text: '请上传图片',
		...
		$scope.edit_group = function(){
			...
			$http.post(
				_c.appPath+'meo/wb_group/edit_group',
				{
					name     : name,
					id    : id
				}
		...
		$scope.get_group_by_id = function(group_id){
			WeiboUserGroup.getFilterGroup({'id':group_id},function(data){
		...
		$scope.show_statistics = function(group_id){
			...
			WeiboUserGroup.get_group_statistics({'group_id':group_id}, function (res) {
				$scope.statisticsData = res;
				//性别highchart
				angular.forEach(res.data.gender, function (v, k) {
					if (k == 1) {
						k = '男';
					};
					if (k == 2) {
				...
				//地区highchart
				angular.forEach(res.data.province_code, function (v, k) {
