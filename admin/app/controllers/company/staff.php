...
class Staff extends ME_Controller {
	...
	{
		$company_id = (int) $this->input->get_post('company_id');
		$list = $this->db->select('s.*, p.name AS position')
			->from('staff s')
			->join('position p', 's.position_id = p.id', 'left')
			->where('s.company_id', $company_id)
			->order_by('s.login_count', 'DESC')
			->get()->result_array();
		...
		if ($list)
			...
			$this->meret(NULL, MERET_EMPTY, '该公司没有设置员工账号！');
	}
